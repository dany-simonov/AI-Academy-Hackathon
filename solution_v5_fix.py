"""
Fix for v5: train temporal model only on T1 (no leakage), validate on T2.
Produces `submission_2.csv` and comparison stats with previous submissions if present.
"""
...
OUT2 = os.path.join(BASE, "submission_2.csv")

TOP_K = 20
_EMPTY = frozenset()

# Use same helper functions/FeatureBuilder as v5 (trimmed)
...
if __name__ == '__main__':
    ...
    # T1: events < val_cut; T2: events >= val_cut
    ...
    fb_t1 = FeatureBuilderFix(T1, users, eds, e2b, e2a, bgm, egm, egs, svd_t1, uenc, eenc)
    fb_all = FeatureBuilderFix(inter, users, eds, e2b, e2a, bgm, egm, egs, svd_all, uenc2, eenc2)

    # temporal model TRAIN on T1 only (no T2 labels)
    ...
    X = feat_train[FeatureBuilderFix.COLS].values.astype(np.float32); y = feat_train['label'].values.astype(np.float32)
    ...
    # full model on all interactions
    ...
    Xf = feat_full[FeatureBuilderFix.COLS].values.astype(np.float32); yf = feat_full['label'].values.astype(np.float32)
    ...
    # prepare val candidates: ensure negatives exclude T1 interactions
    ...
    vc_feat['score_temporal'] = model_temporal.predict(vc_feat[FeatureBuilderFix.COLS].values.astype(np.float32))
    vc_feat['score_full'] = model_full.predict(fb_all.featurize(val_cands[['user_id','edition_id']])[FeatureBuilderFix.COLS].values.astype(np.float32))
    ...
    print('Starting grid search (fixed)')
        ...
        for lam in lambdas:
            for pool in pools:
                ...
                if best is None or sc > best[0]:
                    best = (sc, w, lam, pool)
    print('Best on val (fixed):', best)

    # score competition candidates with best params
    s1c = fb_all.featurize(cands[['user_id','edition_id']])
    s1c['score_temporal'] = model_temporal.predict(fb_all.featurize(cands[['user_id','edition_id']])[FeatureBuilderFix.COLS].values.astype(np.float32))
    s1c['score_full'] = model_full.predict(fb_all.featurize(cands[['user_id','edition_id']])[FeatureBuilderFix.COLS].values.astype(np.float32))
    ...
    final_sub.to_csv(OUT2, index=False)
    import hashlib
    md5 = hashlib.md5(open(OUT2,'rb').read()).hexdigest()
    print('Saved', OUT2, 'md5', md5)

    # compare with existing submissions if present
    def topk_map(path):
        if not os.path.exists(path): return {}
        df = pd.read_csv(path)
        return df.groupby('user_id')['edition_id'].apply(list).to_dict()
    s_old = topk_map(os.path.join(BASE, 'submission.csv'))
    s1 = topk_map(os.path.join(BASE, 'submission_1.csv'))
    s2 = topk_map(OUT2)
    def overlap(a,b):
        users = set(a.keys()) & set(b.keys())
        tot=0; match=0
        for u in users:
            A=a[u][:TOP_K]; B=b[u][:TOP_K]
            tot+=TOP_K
            match+=len(set(A)&set(B))
        return match/tot if tot else 0
    print('Overlap submission.csv vs submission_1.csv:', overlap(s_old,s1))
    print('Overlap submission.csv vs submission_2.csv:', overlap(s_old,s2))
    print('Overlap submission_1.csv vs submission_2.csv:', overlap(s1,s2))
