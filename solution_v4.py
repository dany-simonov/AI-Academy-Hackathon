"""
BookRec Hackathon — Solution v4  (multi-k SVD + fine λ tuning + ensemble)
=========================================================================
Improvements over v3:
  - Three SVD signals: k=32, k=64, k=128  (different granularity)
  - Fine-grained λ search around 0.95
  - Ensemble: temporal + full-data model
"""
...
def load_data():
    ...
    print(f"[load] {len(inter):,} inter  {len(targets):,} users  "
          f"{len(cands):,} cands  ({time.time()-t0:.1f}s)")
...
def do_svd(inter, all_u, all_e, k=64, half_life=180):
    ...
    return U*s[np.newaxis,:],Vt.T,u_enc,e_enc,R

def multi_svd(inter, all_u, all_e):
    """Compute SVD for k=32,64,128 and return (user_factors_list, item_factors_list, u_enc, e_enc)"""
    results = {}
    for k in [32, 64, 128]:
        uf,itf,ue,ee,R = do_svd(inter,all_u,all_e,k=k)
        results[k] = (uf,itf)
        print(f"   SVD k={k} done  shape={R.shape} nnz={R.nnz:,}")
    # u_enc / e_enc are the same for all k
    return results, ue, ee
...
class FeatureBuilder:
    ...
    def __init__(self, inter, users, eds, e2b, e2a, bgm, egm, egs,
        ...
        ugp={};ugs={};self.u_genre_rr={}
        for uid,grp in im.groupby("user_id"):
            gc=defaultdict(float);gt=defaultdict(float);gr=defaultdict(float)
            for et,bid in zip(grp["event_type"],grp["book_id"]):
                if pd.notna(bid):
                    for g in bgm.get(int(bid),_EMPTY):
                        gc[g]+=(3 if et==2 else 1);gt[g]+=1
                        if et==2: gr[g]+=1
            ugp[uid]=dict(gc);ugs[uid]=set(gc.keys())
            for g in gt: self.u_genre_rr[(uid,g)]=gr[g]/gt[g]
        rows=[]
    ...
    def featurize(self, pairs_df):
        ...
        for i in range(N):
            ...
            rates=[self.u_genre_rr.get((uid,g),0) for g in ig]
...
# ─────────────────────────────────────────────────────────────────────────────
def make_training(src_inter,lbl_inter,fb):
    t0=time.time()
    lbl=lbl_inter.copy();lbl["rel"]=lbl["event_type"].map({1:1.0,2:3.0})
    ...
    for uid,eid in zip(lbl_inter["user_id"],lbl_inter["edition_id"]):u_all[uid].add(eid)
    negs=[]
...
# ─────────────────────────────────────────────────────────────────────────────
def mmr_rerank(scored_df,egm,lam,pool=MMR_POOL,top_k=TOP_K):
    results=[]
        ...
        for _ in range(top_k):
            ...
            for idx in rem:
                ...
                if sel:
                    ds=0.0
                    for si in sel:
                        sg=gsets[si];u=len(ig|sg);ds+=(1-len(ig&sg)/u) if u else 0
                    ad=ds/len(sel)
                else:ad=1.0
...
def compute_metrics(sub,gt_dict,egm,target_users):
    ...
    for uid in target_users:
        ...
        for k_ in range(TOP_K):
            if trel[k_] and gsets[k_]:cv+=w[k_]*len(gsets[k_]-cov)/len(gsets[k_]);cov|=gsets[k_]
        coverage=cv/ws if ws else 0
        ri=[k_ for k_ in range(TOP_K) if trel[k_]]
        if len(ri)<2:ild=0.0
        else:
            ds=0.0;cnt=0
            for i in range(len(ri)):
                for j in range(i+1,len(ri)):
                    gi,gj=gsets[ri[i]],gsets[ri[j]];u=len(gi|gj)
                    ds+=(1-len(gi&gj)/u) if u else 0;cnt+=1
...
def main():
    wall=time.time()
    print("="*60)
    print("  BookRec v4 — multi-k SVD + λ tuning + ensemble")
    print("="*60)
    ...
    # ══════════════════════════════════════════════════════════════════════════
    #  MODEL 1: Temporal (T1 features → T2 labels, multi-k SVD)
    # ══════════════════════════════════════════════════════════════════════════
    print("\n── Model 1: temporal (multi-k SVD) ──")
    print("[multi-svd T1]")
    svd_t1,ue1,ee1=multi_svd(T1,all_u,all_e)
    print("[features T1]")
    fb1=FeatureBuilder(T1,users,eds,e2b,e2a,bgm,egm,egs,svd_t1,ue1,ee1)
    print("[train T1→T2]")
    tf1=make_training(T1,T2,fb1)
    model_t=train_gbm(tf1,max_iter=1500)
    ...
    print(f"[val candidates] {len(vc_df):,}")
    vc_feat=fb1.featurize(vc_df[["user_id","edition_id"]])
    vc_feat["score"]=model_t.predict(vc_feat[FeatureBuilder.COLS].values.astype(np.float32))
    vu=set(vc_df["user_id"])&set(gt["user_id"])

    best_sc=-1;best_lam=0.95
    print("\n[λ tuning]")
    for lam in [0.85,0.90,0.92,0.94,0.95,0.96,0.97,0.98,1.0]:
        vs=mmr_rerank(vc_feat,egm,lam=lam)
        mn,md,sc=compute_metrics(vs,gt_dict,egm,vu)
        tag=" ◄" if sc>best_sc else ""
        if sc>best_sc:best_sc=sc;best_lam=lam
        print(f"   λ={lam:.2f}  NDCG={mn:.5f}  Div={md:.5f}  Score={sc:.5f}{tag}")
    print(f"\n   → Best λ={best_lam:.2f}  Score={best_sc:.5f}")

    # ══════════════════════════════════════════════════════════════════════════
    #  MODEL 2: Full-data
    # ══════════════════════════════════════════════════════════════════════════
    print("\n── Model 2: full-data (multi-k SVD) ──")
    print("[multi-svd full]")
    svd_all,uea,eea=multi_svd(inter,all_u,all_e)
    print("[features full]")
    fb_all=FeatureBuilder(inter,users,eds,e2b,e2a,bgm,egm,egs,svd_all,uea,eea)
    print("[train full]")
    tmp=inter.copy();tmp["rel"]=tmp["event_type"].map({1:1.0,2:3.0})
    pos_f=tmp.groupby(["user_id","edition_id"])["rel"].max().reset_index().rename(columns={"rel":"label"})
    u_ia=inter.groupby("user_id")["edition_id"].apply(set).to_dict()
    ae_f=list(set(inter["edition_id"]))
    negs2=[]
    for uid,eset in u_ia.items():
        nn=min(len(eset)*5,500)
        pool=[e for e in ae_f if e not in eset]
        if len(pool)>nn:ch=np.random.choice(pool,nn,replace=False)
        else:ch=pool
        for eid in ch:negs2.append({"user_id":uid,"edition_id":eid,"label":0.0})
    neg2=pd.DataFrame(negs2)
    train_f=pd.concat([pos_f,neg2],ignore_index=True).sample(frac=1,random_state=42)
    print(f"   {len(pos_f):,}p + {len(neg2):,}n = {len(train_f):,}")
    tf_full=fb_all.featurize(train_f[["user_id","edition_id"]])
    tf_full["label"]=train_f["label"].values
    ...
    X_c=cf[FeatureBuilder.COLS].values.astype(np.float32)
    s1=model_t.predict(X_c);s2=model_full.predict(X_c)
