"""
Aggressive exploration: wider grid (neg sampling, pool sizes, weights, lambdas), create submission_3.csv,
and produce per-user analysis comparing submission.csv, submission_1.csv, submission_2.csv, submission_3.csv.
"""
...
OUT3 = os.path.join(BASE, "submission_3.csv")
TOP_K = 20
_EMPTY = frozenset()

# reuse helper functions from fix script (copied)
...
if __name__ == '__main__':
    ...
    fb_t1 = FeatureBuilderX(T1, users, eds, e2b, e2a, bgm, egm, egs, svd_t1, uenc, eenc)
    fb_all = FeatureBuilderX(inter, users, eds, e2b, e2a, bgm, egm, egs, svd_all, uenc2, eenc2)

    # Train temporal on T1 (no leakage) but increase negative sampling drastically
    tmp = T1.copy(); tmp['rel'] = tmp['event_type'].map({1:1.0,2:3.0})
    ...
    for uid in pos['user_id'].unique():
        npos = len(pos[pos['user_id'] == uid]); nneg = min(max(npos * 10, 500), 2000)
        excl = u_all[uid]
    ...
    X = feat_train[FeatureBuilderX.COLS].values.astype(np.float32); y = feat_train['label'].values.astype(np.float32)
    model_temporal = HistGradientBoostingRegressor(max_iter=1500, max_depth=9, learning_rate=0.03, min_samples_leaf=20, l2_regularization=1.0, max_bins=255, random_state=42, early_stopping=True, validation_fraction=0.1, n_iter_no_change=50)
    model_temporal.fit(X,y)

    # full model with stronger negatives too
    ...
    for uid, eset in u_inter_all.items():
        nn = min(max(len(eset) * 10, 500), 2000)
    ...
    Xf = feat_full[FeatureBuilderX.COLS].values.astype(np.float32); yf = feat_full['label'].values.astype(np.float32)
    model_full = HistGradientBoostingRegressor(max_iter=2000, max_depth=9, learning_rate=0.03, min_samples_leaf=20, l2_regularization=1.0, max_bins=255, random_state=42, early_stopping=True, validation_fraction=0.1, n_iter_no_change=75)
    model_full.fit(Xf, yf)

    # build val candidates with larger negative pools
    ...
    vc_feat['score_temporal'] = model_temporal.predict(vc_feat[FeatureBuilderX.COLS].values.astype(np.float32))
    vc_feat['score_full'] = model_full.predict(fb_all.featurize(val_cands[['user_id','edition_id']])[FeatureBuilderX.COLS].values.astype(np.float32))

    # grid - much wider
    ensemble_weights = [0.0, 0.3, 0.5, 0.7, 1.0]
    lambdas = [0.7, 0.8, 0.85, 0.9, 0.94, 0.96]
    pools = [80, 150, 200]
    best = None
    ...
    for w in ensemble_weights:
        for lam in lambdas:
            for pool in pools:
                ...
                s1n = (s1 - s1.min()) / (s1.max() - s1.min() + 1e-12) if (s1.max() - s1.min())>0 else s1
                s2n = (s2 - s2.min()) / (s2.max() - s2.min() + 1e-12) if (s2.max() - s2.min())>0 else s2
                ...
                if best is None or sc > best[0]:
                    best = (sc, w, lam, pool)
    print('Best aggressive on val:', best)

    # score competition candidates with best params
    s1c = fb_all.featurize(cands[['user_id','edition_id']])
    s1c['score_temporal'] = model_temporal.predict(fb_all.featurize(cands[['user_id','edition_id']])[FeatureBuilderX.COLS].values.astype(np.float32))
    s1c['score_full'] = model_full.predict(fb_all.featurize(cands[['user_id','edition_id']])[FeatureBuilderX.COLS].values.astype(np.float32))
    ...
    final_sub.to_csv(OUT3, index=False)
    import hashlib
    md5 = hashlib.md5(open(OUT3,'rb').read()).hexdigest()
    print('Saved', OUT3, 'md5', md5)

    # load existing submissions and compute per-user ndcg on gt
    def load_sub(path):
        if not os.path.exists(path): return {}
        df = pd.read_csv(path)
        return df
    s0 = load_sub(os.path.join(BASE, 'submission.csv'))
    s1 = load_sub(os.path.join(BASE, 'submission_1.csv'))
    s2 = load_sub(os.path.join(BASE, 'submission_2.csv'))
    s3 = final_sub

    # user-level ndcg for each submission
    def user_ndcg(df):
        res = {}
        for uid, g in df.groupby('user_id'):
            ranked = g.sort_values('rank')['edition_id'].values[:TOP_K]
            rels = [gt_dict.get((uid, int(e)), 0) for e in ranked]
            dcg = sum(r / math.log2(k+2) for k, r in enumerate(rels))
            all_r = sorted([v for (u,e), v in gt_dict.items() if u == uid], reverse=True)
            idcg = sum(r / math.log2(k+2) for k, r in enumerate((all_r + [0]*TOP_K)[:TOP_K]))
            res[uid] = dcg / idcg if idcg>0 else 0
        return res
    u0 = user_ndcg(s0) if isinstance(s0, pd.DataFrame) else {}
    u1 = user_ndcg(s1) if isinstance(s1, pd.DataFrame) else {}
    u2 = user_ndcg(s2) if isinstance(s2, pd.DataFrame) else {}
    u3 = user_ndcg(s3)

    users = sorted(set(u3.keys()) | set(u2.keys()) | set(u1.keys()) | set(u0.keys()))
    diffs = []
    for u in users:
        a = u3.get(u,0); b = u2.get(u,0)
        diffs.append((u, a-b, a, b))
    diffs.sort(key=lambda x: x[1], reverse=True)
    top_improve = diffs[:50]
    top_worse = diffs[-50:]
    print('\nTop 10 users where submission_3 improved vs submission_2:')
    for u,d,a,b in top_improve[:10]:
        print(u, 'delta_ndcg=', d, 'ndcg3=', a, 'ndcg2=', b)
    print('\nTop 10 users where submission_3 got worse vs submission_2:')
    for u,d,a,b in top_worse[-10:]:
        print(u, 'delta_ndcg=', d, 'ndcg3=', a, 'ndcg2=', b)

    # overlaps
    def overlap(a,b):
        Au = a.groupby('user_id')['edition_id'].apply(list).to_dict()
        Bu = b.groupby('user_id')['edition_id'].apply(list).to_dict()
        users = set(Au.keys()) & set(Bu.keys())
        tot=0; match=0
        for u in users:
            A=Au[u][:TOP_K]; B=Bu[u][:TOP_K]
            tot+=TOP_K
            match+=len(set(A)&set(B))
        return match/tot if tot else 0
    ov_0_3 = overlap(s0, s3) if isinstance(s0, pd.DataFrame) else None
    ov_1_3 = overlap(s1, s3) if isinstance(s1, pd.DataFrame) else None
    ov_2_3 = overlap(s2, s3)
    print('\nOverlaps with submission_3:')
    print('submission vs submission_3:', ov_0_3)
    print('submission_1 vs submission_3:', ov_1_3)
    print('submission_2 vs submission_3:', ov_2_3)
