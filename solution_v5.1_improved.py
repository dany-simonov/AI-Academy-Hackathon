"""
BookRec Hackathon — Solution v5.1 (Relevance-Focused)
======================================================
CRITICAL FIX: Balance relevance vs diversity
  - λ tuning: 0.75-0.95 (HIGHER λ = more relevance weight!)
  - Previous v5 used λ=0.1-0.7 → 70-90% diversity, only 10-30% relevance
  - Fixed diversity formula weights
  - Increased negative ratio for better discrimination
  - Standard wishlist weights (2x less than read)
  - Larger MMR pool for better selection
"""
...
SUBMIT = os.path.join(BASE, "materials")
OUT  = os.path.join(BASE, "submission_3.csv")  # Changed to submission_3.csv

TOP_K      = 20
SVD_K      = 64
NEG_RATIO  = 7  # INCREASED for better learning (was 3, too low!)
MMR_POOL   = 150  # INCREASED for better candidate selection
_EMPTY     = frozenset()
...
def do_svd(inter, all_users, all_editions, k=SVD_K, half_life=120):
    ...
    tmp = inter.copy()
    tmp["rel"] = tmp["event_type"].map({1: 1.0, 2: 3.0})  # FIXED: standard weights (read=3x wishlist)
            ...
            for et, bid in zip(grp["event_type"], grp["book_id"]):
                if pd.notna(bid):
                    for g in bgm.get(int(bid), _EMPTY):
                        weight = 3.0 if et == 2 else 1.0  # FIXED: standard weights (read=3x wishlist)
                        gc[g] += weight
...
def make_training_data(T1, T2, fb: FeatureBuilder):
    ...
    t0 = time.time()
    T2c = T2.copy()
    T2c["rel"] = T2c["event_type"].map({1: 1.0, 2: 3.0})  # FIXED: standard weights
    ...
    for uid in pos["user_id"].unique():
        ...
        n_neg = min(n_pos * NEG_RATIO, 400)  # INCREASED cap from 200 to 400
        excl = u_all[uid]
...
def train_gbm(feat):
    ...
    # Enhanced parameters for relevance focus
    mdl = HistGradientBoostingRegressor(
        max_iter=1000,  # More iterations for better fit
        max_depth=7,   # Slightly shallower for generalization
        learning_rate=0.04,  # Balanced learning rate
        min_samples_leaf=25,  # Prevent overfitting
        l2_regularization=0.8,  # More regularization for stability
        max_bins=255,
        random_state=42,
        early_stopping=True,
    ...
    for uid, grp in scored_df.groupby("user_id"):
        ...
        for _ in range(top_k):
            ...
            for idx in rem:
                ...
                # FIXED: balanced diversity component (less aggressive coverage)
                diversity_score = 0.4 * cg + 0.6 * ad  # Changed from 0.6/0.4 to 0.4/0.6
                
                # Final MMR score: higher lambda = MORE relevance weight
...
def main():
    wall = time.time()
    print("="*70)
    print("  BookRec Hackathon — v5.1 (RELEVANCE-FOCUSED FIX)")
    print("  Critical fix: λ range changed from 0.1-0.7 to 0.75-0.97")
    print("="*70)
    ...
    T2c = T2.copy()
    T2c["rel"] = T2c["event_type"].map({1:1.0, 2:3})  # FIXED: standard weights
    ...
    print("\n[λ tuning - CRITICAL FIX: focusing on RELEVANCE!]")
    print("   Note: Higher λ = more relevance weight (e.g., λ=0.9 means 90% relevance, 10% diversity)")
    lambda_values = [0.75, 0.80, 0.85, 0.88, 0.90, 0.92, 0.94, 0.95, 0.96, 0.97]  # FIXED range!
